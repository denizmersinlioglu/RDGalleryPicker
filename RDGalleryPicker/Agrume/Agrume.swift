...
public final class Agrume: UIViewController {
  ...
  public func show(from viewController: UIViewController, color: UIColor) {
  ...
  private func addSubviews(color: UIColor) {
    ...
    let backgroundPadder = UIView(frame: view.frame)
    backgroundPadder.backgroundColor = color
    view.addSubview(backgroundPadder)
...
//    closeButton.anchor(top: view.topAnchor, left: view.leftAnchor, bottom: nil, right: nil, paddingTop: 5, paddingLeft: 5, paddingBottom: 0, paddingRight: 0, width: 44, height: 44)
  }
...
extension Agrume: AgrumeDataSource {
  ...
  public func image(forIndex index: Int, completion: @escaping (UIImage?) -> Void) {
    if let handler = AgrumeServiceLocator.shared.downloadHandler, let url = images[index].url {
      ...
      downloadTask = AgrumeImageDownloader.downloadImage(url, completion: completion)
    } else {
